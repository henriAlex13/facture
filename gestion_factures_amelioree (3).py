...
def export_base_centrale(df):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        cell = worksheet['A1']
        cell.value = "📊 BASE CENTRALE - COCODY"
...
if page == "📊 Base Centrale":
                        ...
                        with st.spinner("⏳ Import BT en cours..."):
                            ...
                            if df_template_bt is None:
                                ...
                                for _, row_facture in df_bt.iterrows():
                                    ...
                                    ligne_template = df_template_bt[df_template_bt['IDENTIFIANT'].astype(str) == ref_contrat]
                                    
                                    if not ligne_centrale.empty:
                                        if not ligne_template.empty:
                                            ligne_base = ligne_template.iloc[0].copy()
                                        else:
                                            ligne_base = pd.Series({
                                                'COMPTE DE CHARGES': '600218464',
                                                'MONTANT': 0,
                                                'CODE PART LIB': '',
                                                'CODE_CHARGE': 'D',
                                                'CENTRE': 'xxxxxxxxxx',
                                                'INTER.2': 'xxxxxxx',
                                                'HV': 'xxxxxx',
                                                'LIBELLE COMPLEMENTAIRE': f"CIE BT {ligne_centrale.iloc[0]['SITES']}",
                                                'JIB Calc': '',
                                                'Vérification compte': 'FACTURAT ELECTRICITE D\'EXPL',
                                                'IDENTIFIANT': ref_contrat
                                            })
                                        
                                        ligne_base['UC'] = ligne_centrale.iloc[0]['UC']
                                        ligne_base['SITES'] = ligne_centrale.iloc[0]['SITES']
                                        ligne_base['TENSION'] = ligne_centrale.iloc[0]['TENSION']
                        ...
                        with st.spinner("⏳ Import HT en cours..."):
                            ...
                            if df_template_ht is None:
                                ...
                                for _, row_facture in df_ht.iterrows():
                                    ...
                                    ligne_template = df_template_ht[df_template_ht['IDENTIFIANT'].astype(str) == refraccord]
                                    ...
                                    if not ligne_centrale.empty:
                                        if not ligne_template.empty:
                                            ...
                                            ligne_base = pd.Series({
                                                'CODE': '711',
                                                'COMPTE DE CHARGES': '60062183464',
                                                'SENS': 'D',
                                                'MONTANT': 0,
                                                'CODE PAYT': '',
                                                'TYPE DEP': '',
                                                'MATR OBJ': '',
                                                'LIBELLE COMPLEMENTAIRE': f"CIE HT {ligne_centrale.iloc[0]['SITES']}",
    ...
    with tab_bt:
        ...
        if df_template_bt is None:
            ...
            with col_stat1:
                st.metric("📝 Lignes", len(df_template_bt))
            with col_stat2:
                if 'MONTANT' in df_template_bt.columns:
                    total_bt = pd.to_numeric(df_template_bt['MONTANT'], errors='coerce').fillna(0).sum()
            ...
            with col1:
                if st.button("💾 Sauvegarder", type="primary", use_container_width=True, key="save_bt_gen"):
                    edited_bt.to_excel(FICHIER_TEMPLATE_BT, index=False)
                    st.success("✅ Template BT sauvegardé !")
                    st.rerun()
            
            with col2:
                # Supprimer IDENTIFIANT si présent
                df_export_bt = edited_bt.drop(columns=['IDENTIFIANT']) if 'IDENTIFIANT' in edited_bt.columns else edited_bt
                
                # Export avec signatures - style BT bleu
                excel_data_bt = export_factures_cie(df_export_bt, type_tension="BT")
                
                st.download_button(
                    "📥 Télécharger Excel BT avec signatures",
                    data=excel_data_bt,
                    file_name=f"FACTURAT_ELECTRICITE_BT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                    key="dl_bt_gen"
                )
    ...
    with tab_ht:
        ...
        if df_template_ht is None:
            ...
            col_stat1, col_stat2 = st.columns(2)
            with col_stat1:
                st.metric("📝 Lignes", len(df_template_ht))
            with col_stat2:
                if 'MONTANT' in df_template_ht.columns:
                    total_ht = pd.to_numeric(df_template_ht['MONTANT'], errors='coerce').fillna(0).sum()
            ...
            with col1:
                if st.button("💾 Sauvegarder", type="primary", use_container_width=True, key="save_ht_gen"):
                    edited_ht.to_excel(FICHIER_TEMPLATE_HT, index=False)
                    st.success("✅ Template HT sauvegardé !")
                    st.rerun()
            
            with col2:
                # Supprimer IDENTIFIANT si présent
                df_export_ht = edited_ht.drop(columns=['IDENTIFIANT']) if 'IDENTIFIANT' in edited_ht.columns else edited_ht
                
                # Export avec signatures - style HT orange
                excel_data_ht = export_factures_cie(df_export_ht, type_tension="HT")
                
                st.download_button(
                    "📥 Télécharger Excel HT avec signatures",
                    data=excel_data_ht,
                    file_name=f"FACTURAT_ELECTRICITE_HT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                    key="dl_ht_gen"
                )
...
<div style='text-align: center; padding: 2rem; color: #666;'>
    <p><strong>Système Centralisé</strong> - Version 3.2 - Export Professionnel avec Signatures</p>
</div>
""", unsafe_allow_html=True)
